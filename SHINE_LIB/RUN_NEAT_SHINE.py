...
def test_NEAT_SHINE(resdir, **kw):
    pop,select, max_val = Neuro_Evolution(resdir, Selector_NEAT_SHINE, **kw)
    select.save_stats(resdir)
    return pop, select, max_val



class NEAT_SHINE(Experience):
    
    def __init__(self):
        super().__init__()

    def run(self):
        test_NEAT_SHINE(resdir = self.make_resdir(algorithm=type(self).__name__), **self.kw)

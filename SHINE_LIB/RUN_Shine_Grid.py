...
class SHINE_Experience(Experience):
    ...
    def run(self):
        ...
        for alpha in [7,9]:
            for beta in [10,90,50,30]:
                st = []
                for i in range(1):
                    print("Experimentation for : ",alpha," and ", beta)
                    print("=============")
                    pop, select, max_val = test_SHINE(alpha,beta, self.make_resdir(alpha=alpha, beta=beta) ,ngen=100, **self.kw)
                    st.append(max_val)
                st = np.array(st)
                f.write(str(alpha) + ", "+ str(beta) + " , " + " : " + str(st.mean()) + " , " + str(st.std()) + "\n" )
                perf_dict[(alpha,beta)] = (st.mean(), st.std())
